...
def compare_ffs(in_dict, conf_id_tag, out_prefix, keep_ref_conf=False, mol_slice=None):
    ...
    for ff_label, ff_dict in in_dict.items():
        ...
        ref_en = defaultdict(list)
        que_en = defaultdict(list)
        rms_en = defaultdict(list)
        tfd_en = defaultdict(list)
        smi_en = defaultdict(list)
        # I was getting each conformer as a separate molecule, causing empty
        # arrays from remove lowest. this aims to counteract that by grouping
        # by molecule title (ie "full_1")
        ...
        for name in ref_en.keys():
            enes_ref = ref_en[name]
            enes_que = que_en[name]
            rmsds_mol = rms_en[name]
